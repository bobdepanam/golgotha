...
import { Stats, useKeyboardControls, useProgress } from "@react-three/drei";
...
export function InfiniteCanvasSceneContent({
  media,
  onTextureProgress,
  showFps = false,
  cameraFov = 60,
  ...
  enabled = true,
  onSelect,
}: InfiniteCanvasProps & { enabled?: boolean; onSelect?: (item: MediaItem) => void }) {
  const isTouchDevice = useIsTouchDevice();
  ...
  return (
    <>
      ...
      <SceneController media={media} onTextureProgress={onTextureProgress} enabled={enabled} onSelect={onSelect} />
