...
type ParallaxSection = {
  imageSrc?: string;
  videoSrc?: string;
  /** ✅ image d’affiche pendant le chargement/avant lecture */
  poster?: string;
...
function TextParallaxContent({ section }: { section: ParallaxSection }) {
  return (
    <div className={styles.section} style={{ paddingLeft: IMG_PADDING, paddingRight: IMG_PADDING }}>
      <StickyMedia imageSrc={section.imageSrc} videoSrc={section.videoSrc} poster={section.poster}>
  ...
  return (
    ...
    >
      ...
      {videoSrc && (
        <video
          src={videoSrc}
          poster={poster}                            // ✅ utilise le poster
          autoPlay
          muted
          loop
          playsInline
          preload="metadata"                         // ✅ évite l’écran noir
